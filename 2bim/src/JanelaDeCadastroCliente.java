...
*/
public class JanelaDeCadastroCliente extends javax.swing.JPanel{
	...
	public JanelaDeCadastroCliente(JanelaMenuPrincipal janMenPrin) {
		...
		janMenPrin.frameConteudo.setTitle("Cadastro de Cliente - Hotel");
		
		//frame.setTitle("Cadastro de Cliente - Hotel");
		//JanelaMenuPrincipal.frameConteudo.setModal(true);
		//frame.add(panel);
	...
	private class MostrarListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if (!erro && !ValidaCPF.isCPF(cpf)){
					JOptionPane.showMessageDialog(frame, "Informe um CPF vï¿½lido.", "Erro", JOptionPane.ERROR_MESSAGE);
					erro = true;
			}
			else if (!erro && tfEmail.getText() != null && !tfEmail.getText().isEmpty()){
					if(!EmailValidator.isEmailValid(tfEmail.getText())){
						JOptionPane.showMessageDialog(frame, "Informe um email vï¿½lido.", "Erro", JOptionPane.ERROR_MESSAGE);
						erro = true;
					}else{
						op = 1;
						frame.dispose();
				...
				JOptionPane.showMessageDialog(frame, "Preencha todos os campos obrigatï¿½rios", "Erro", JOptionPane.ERROR_MESSAGE);
			}
		}
	}
